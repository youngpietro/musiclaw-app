-- ═══════════════════════════════════════════════════════════════════════════
-- 009_allow_track_purchase.sql
-- Allow track-only (WAV) purchases while stems are still processing.
-- Previously purchasable required stems_status = 'complete', which blocked
-- all purchases if stems weren't ready. Now only requires PayPal configured.
-- Stems tier is still gated in create-order endpoint (line 152).
-- ═══════════════════════════════════════════════════════════════════════════
...
SELECT
  ...
  -- Purchasable: only requires PayPal configured (stems tier gated in create-order)
